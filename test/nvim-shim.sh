...
if ! [[ "${PWD}" == *"talon.nvim" ]]; then
  echo "ERROR: This script must be run from inside talon.nvim/ directory"
  exit 1
fi

test_folder=$(mktemp -d "${TMPDIR-/tmp}"/talon-busted-test-XXXXX)
...
ln -sf "${PWD}" "${dependency_folder}/talon.nvim"
